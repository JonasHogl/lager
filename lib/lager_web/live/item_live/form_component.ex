defmodule LagerWeb.ItemLive.FormComponent do
  ...
  def update(%{item: item} = assigns, socket) do
    {:ok,
     ...
     |> assign(assigns)
     |> assign_new(:form, fn ->
       to_form(Storage.change_item(item))
     end)}
  end

  @impl true
  def handle_event("validate", %{"item" => item_params}, socket) do
    changeset = Storage.change_item(socket.assigns.item, item_params)
  ...
  defp save_item(socket, :edit, item_params) do
    case Storage.update_item(socket.assigns.item, item_params) do
      {:ok, item} ->
