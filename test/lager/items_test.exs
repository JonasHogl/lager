defmodule Lager.ItemsTest do
  ...
  describe "items" do
    ...
    test "create_item/1 with valid data creates a item" do
      valid_attrs = %{acquired: ~D[2025-03-30], residual_mass: 42, best_before: ~D[2025-03-30]}

      assert {:ok, %Item{} = item} = Items.create_item(valid_attrs)
      assert item.acquired == ~D[2025-03-30]
      assert item.residual_mass == 42
      assert item.best_before == ~D[2025-03-30]
    end
    ...
    test "update_item/2 with valid data updates the item" do
      item = item_fixture()
      update_attrs = %{acquired: ~D[2025-03-31], residual_mass: 43, best_before: ~D[2025-03-31]}

      assert {:ok, %Item{} = item} = Items.update_item(item, update_attrs)
      assert item.acquired == ~D[2025-03-31]
      assert item.residual_mass == 43
      assert item.best_before == ~D[2025-03-31]
    end
